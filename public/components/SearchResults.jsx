...
const SearchResults = ({fetchedRecipes}) => {
  ...
  <div className='sidebar'>
    ...
    {pages > 1 &&
      ...
      <button onClick={prev} className='btn-prev bg-[#f9f5f3] text-[#f38e82] text-[1.3rem] py-[0.5rem] px-[1.8rem] capitalize font-semibold rounded-full flex gap-3 items-center justify-center transition-all  duration-200 hover:bg-[#f3ded4]'><span>prev</span></button>
      <button onClick={(e)=>next(e)} className='btn-next bg-[#f9f5f3] text-[#f38e82] text-[1.3rem] py-[0.5rem] px-[1.8rem] capitalize font-semibold rounded-full flex gap-3 items-center justify-center transition-all duration-200 hover:bg-[#f3ded4] '><span>next</span></button>
      </div>
    }
  </div>
  )
